window.addEventListener("load", function () {
  ...
  canvas.height = 500;
  // audio
  let shotAudio = new Audio('assets/audio/laser-shot.flac')
  let audioTheme = new Audio('assets/audio/main-theme.wav')
  let audioOn = false
  class Inputhandler {
    constructor(game) {
      this.game = game;
      ...
      window.addEventListener("keydown", (e) => {
        ...
        } else if (e.key === "l" && !audioOn){
          audioTheme.currentTime = 3
          audioTheme.play()
          audioOn = true 
        } else if (e.key === 'l' && audioOn){
          audioOn = false 
  ...
  class Player {
    ...
    draw(context) {
      context.fillStyle = "black"; // окрашивает прямоугольник
      context.fillRect(this.x, this.y, this.width, this.height); // рисует прямоугольник
      context.fillText(this.playerLives, this.x, this.y-1)
      this.projectiles.forEach((projectile) => {
  ...
  class Enemy {
    ...
    draw(context) {
      // сохраниие и сброс состояния контекста помогает исполльзовать стили только для элементов внутри
      context.save()
  ...
  class UI {
    ...
    draw(context) {
      ...
      if (this.game.gameOver) {
        ...
        context.font = "72px " + this.fontFamily; // пример изменения размера шрифта у контекста
        let message = "Game Over";
        context.fillText(message, this.game.width * 0.5, this.game.height * 0.5);
        context.strokeText(message, this.game.width * 0.5, this.game.height * 0.5) // контур текста
      }
  ...
  class Game {
    constructor(width, height) {
      ...
      this.ammoTimer = 0; // таймер накопления боеприпасов
        ...
        this.player.projectiles.forEach((projectile) => {
          if (this.checkCollision(projectile, enemy)) {
            ...
            if (enemy.lives <= 0) {
              enemy.markedForDeletion = true;
              this.score++;
              if (this.score >= this.winningScore) { // проверка на колво очков если > конец + удаление всех врагов 
                this.gameOver = true;
                this.enemies.forEach((enemy) => {
                  enemy.markedForDeletion = true 
                })
  ...
  // animation loop
  let lastTime = 0; // эта переменная будет хранить значение времени предыдущей анимации

  function animate(currentTime) {
    const deltaTime = currentTime - lastTime; // разница времени между текущим и предыдущей анимацией
    lastTime = currentTime;
    context.clearRect(0, 0, canvas.width, canvas.height); // очищаем полотно перед след анимацией
    game.draw(context);
    game.update(deltaTime);
    requestAnimationFrame(animate); // запускает функцию анимации примерно 60 раз в секунду
  }
  animate(0);
});
